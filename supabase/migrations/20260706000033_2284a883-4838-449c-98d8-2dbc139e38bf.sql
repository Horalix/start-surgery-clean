CREATE OR REPLACE FUNCTION public.is_battle_participant(_room_id uuid, _user_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.is_battle_waiting(_room_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
WITH CHECK (
  ...
  AND public.is_battle_waiting(room_id)
);
...
USING (
  auth.uid() IS NOT NULL
  AND (
    public.is_battle_waiting(room_id)
    OR public.is_battle_participant(room_id, auth.uid())
  )
);
...
WITH CHECK (
  auth.uid() IS NOT NULL
  AND user_id = auth.uid()
  AND public.is_battle_participant(room_id, auth.uid())
);
