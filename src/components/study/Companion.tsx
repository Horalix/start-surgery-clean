import { useMemo } from "react";
import { stageForLevel, type CompanionMood } from "@/lib/study/companion";
import type { CharacterCustomization } from "@/lib/study/types";
import { cn } from "@/lib/utils";

interface Px {
  x: number;
  y: number;
  w: number;
  h: number;
  c: string;
}

const OUTLINE = "#141c2b";
const MASK = "#eef3f8";
const EYE = "#20293b";

function buildPixels(level: number, mood: CompanionMood, character?: CharacterCustomization): Px[] {
  const st = stageForLevel(level);
  const p = { ...st.palette, ...(character?.palette ?? {}) };
  const props = { ...st.props, ...(character?.props ?? {}) };
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });

  // Body / scrubs
  add(4, 8, 8, 7, p.scrub);
  add(4, 8, 1, 7, p.scrubDark);
  add(11, 8, 1, 7, p.scrubDark);
  add(6, 8, 4, 1, p.scrubDark);
  add(7, 9, 2, 1, p.scrubDark);
  add(3, 9, 1, 4, p.scrub);
  add(12, 9, 1, 4, p.scrub);
  add(3, 13, 1, 1, p.skin);
  add(12, 13, 1, 1, p.skin);
  add(5, 15, 2, 1, p.scrubDark);
  add(9, 15, 2, 1, p.scrubDark);

  if (props.badge) add(9, 10, 1, 1, p.accent);

  // Neck + head
  add(7, 7, 2, 1, p.skin);
  add(5, 3, 6, 5, p.skin);
  add(4, 4, 1, 3, p.skin);
  add(11, 4, 1, 3, p.skin);

  if (props.cap) {
    add(4, 1, 8, 2, p.scrub);
    add(4, 3, 8, 1, p.scrubDark);
    add(3, 2, 1, 1, p.scrubDark);
    add(12, 2, 1, 1, p.scrubDark);
    add(4, 1, 8, 1, MASK);
  } else {
    add(4, 2, 8, 2, p.hair);
    add(4, 3, 1, 2, p.hair);
    add(11, 3, 1, 2, p.hair);
    add(5, 2, 6, 1, p.hair);
  }

  const eyeY = mood === "sad" ? 5 : mood === "thinking" ? 4 : 4;
  if (mood === "happy") {
    add(6, 5, 1, 1, EYE);
    add(9, 5, 1, 1, EYE);
    add(5, 6, 1, 1, p.accent);
    add(10, 6, 1, 1, p.accent);
  } else if (mood === "sad") {
    add(6, 5, 1, 1, EYE);
    add(9, 5, 1, 1, EYE);
    add(6, 4, 1, 1, p.skin);
  } else {
    add(6, eyeY, 1, 1, EYE);
    add(9, eyeY, 1, 1, EYE);
  }

  if (props.glasses) {
    add(5, 4, 2, 1, OUTLINE);
    add(9, 4, 2, 1, OUTLINE);
    add(8, 4, 1, 1, OUTLINE);
    if (props.loupe) add(9, 4, 1, 1, p.accent);
  }

  if (props.mask) {
    add(5, 6, 6, 2, MASK);
    add(4, 6, 1, 1, MASK);
    add(11, 6, 1, 1, MASK);
    add(5, 7, 6, 1, "#dfe7ef");
  } else {
    if (mood === "happy") {
      add(6, 6, 4, 1, "#b45c52");
      add(7, 7, 2, 1, "#b45c52");
    } else if (mood === "sad") {
      add(7, 7, 2, 1, "#b45c52");
      add(6, 8, 1, 1, "#b45c52");
      add(9, 8, 1, 1, "#b45c52");
    } else {
      add(7, 6, 2, 1, "#b45c52");
    }
  }

  return px;
}

// ─── ANGEL Tier I: Cherub ──────────────────────────────────────────────────
function buildAngelT1(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#ffe6cc", SKIN_S = "#e9c9a7";
  const HAIR = "#f6d874", HAIR_L = "#fff2b0";
  const ROBE = "#ffffff", ROBE_S = "#e6ecf5";
  const GOLD = "#f2c94c", GOLD_D = "#b8860b", GOLD_L = "#fff2a8";
  const HALO = "#fff29a";
  const LIP = "#c97a7a";

  // Wings
  add(-1, 6, 1, 5, GOLD_L);
  add(0, 5, 1, 7, ROBE_S);
  add(1, 4, 1, 9, ROBE);
  add(2, 5, 1, 8, ROBE_S);
  add(3, 7, 1, 5, ROBE);
  add(1, 4, 1, 1, GOLD_L);
  add(0, 5, 1, 1, GOLD);
  add(1, 6, 1, 1, GOLD_L);
  add(1, 8, 1, 1, GOLD_L);
  add(1, 10, 1, 1, GOLD_L);
  add(2, 7, 1, 1, GOLD_L);
  add(2, 9, 1, 1, GOLD_L);
  add(2, 11, 1, 1, GOLD_L);
  add(16, 6, 1, 5, GOLD_L);
  add(15, 5, 1, 7, ROBE_S);
  add(14, 4, 1, 9, ROBE);
  add(13, 5, 1, 8, ROBE_S);
  add(12, 7, 1, 5, ROBE);
  add(14, 4, 1, 1, GOLD_L);
  add(15, 5, 1, 1, GOLD);
  add(14, 6, 1, 1, GOLD_L);
  add(14, 8, 1, 1, GOLD_L);
  add(14, 10, 1, 1, GOLD_L);
  add(13, 7, 1, 1, GOLD_L);
  add(13, 9, 1, 1, GOLD_L);
  add(13, 11, 1, 1, GOLD_L);

  add(4, 8, 8, 7, ROBE);
  add(4, 8, 1, 7, ROBE_S);
  add(11, 8, 1, 7, ROBE_S);
  add(4, 10, 8, 1, GOLD);
  add(4, 11, 8, 1, GOLD_D);
  add(7, 10, 2, 1, GOLD_L);
  add(7, 11, 2, 1, "#7ec8f5");
  add(6, 8, 4, 1, ROBE_S);
  add(7, 9, 2, 1, SKIN);
  add(3, 9, 1, 5, ROBE);
  add(12, 9, 1, 5, ROBE);
  add(2, 12, 1, 2, ROBE_S);
  add(13, 12, 1, 2, ROBE_S);
  add(3, 13, 1, 1, SKIN);
  add(12, 13, 1, 1, SKIN);
  add(3, 15, 10, 1, ROBE_S);
  add(4, 16, 8, 1, GOLD);

  add(7, 7, 2, 1, SKIN);
  add(5, 3, 6, 5, SKIN);
  add(4, 4, 1, 3, SKIN);
  add(11, 4, 1, 3, SKIN);
  add(5, 7, 1, 1, SKIN_S);
  add(10, 7, 1, 1, SKIN_S);

  add(4, 2, 8, 2, HAIR);
  add(4, 3, 1, 2, HAIR);
  add(11, 3, 1, 2, HAIR);
  add(5, 2, 6, 1, HAIR_L);
  add(7, 3, 2, 1, HAIR_L);
  add(3, 4, 1, 1, HAIR);
  add(12, 4, 1, 1, HAIR);

  add(4, 0, 8, 1, HALO);
  add(3, 1, 1, 1, GOLD);
  add(12, 1, 1, 1, GOLD);
  add(5, 0, 6, 1, GOLD_L);
  add(6, 1, 4, 1, HALO);

  const eyeC = "#3b6fb0";
  add(6, 5, 1, 1, eyeC);
  add(9, 5, 1, 1, eyeC);
  if (mood === "happy") {
    add(5, 6, 1, 1, "#f5b6b6");
    add(10, 6, 1, 1, "#f5b6b6");
  }
  add(6, 4, 1, 1, "#ffffff");
  add(9, 4, 1, 1, "#ffffff");
  add(7, 6, 2, 1, LIP);

  return px;
}

// ─── ANGEL Tier II: Seraph — six wings, rune halo, twin orbs ────────────────
function buildAngelT2(mood: CompanionMood): Px[] {
  const px = buildAngelT1(mood);
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const GOLD_L = "#fff2a8", GOLD = "#f2c94c", CYAN = "#a0e3ff", WHITE = "#ffffff";
  // Upper wing pair (outstretched further)
  add(-1, 2, 1, 3, GOLD_L);
  add(0, 1, 1, 4, WHITE);
  add(1, 2, 1, 3, "#e6ecf5");
  add(16, 2, 1, 3, GOLD_L);
  add(15, 1, 1, 4, WHITE);
  add(14, 2, 1, 3, "#e6ecf5");
  // Lower wing pair
  add(0, 13, 1, 2, WHITE);
  add(1, 13, 1, 2, "#e6ecf5");
  add(15, 13, 1, 2, WHITE);
  add(14, 13, 1, 2, "#e6ecf5");
  // Twin light orbs orbiting shoulders
  add(2, 5, 1, 1, CYAN);
  add(13, 5, 1, 1, CYAN);
  add(2, 4, 1, 1, WHITE);
  add(13, 4, 1, 1, WHITE);
  // Rune halo (extra ring above)
  add(3, -1, 10, 1, GOLD);
  add(5, -1, 1, 1, WHITE);
  add(8, -1, 1, 1, WHITE);
  add(11, -1, 1, 1, WHITE);
  return px;
}

// ─── ANGEL Tier III: Archangel — armored, huge wings, holy sword ────────────
function buildAngelT3(mood: CompanionMood): Px[] {
  const px = buildAngelT2(mood);
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const GOLD = "#f2c94c", GOLD_L = "#fff2a8", GOLD_D = "#b8860b";
  const STEEL = "#e8ecf2";
  // Golden pauldrons + breastplate overlay
  add(3, 8, 2, 1, GOLD);
  add(11, 8, 2, 1, GOLD);
  add(3, 8, 1, 1, GOLD_L);
  add(12, 8, 1, 1, GOLD_L);
  add(5, 9, 6, 1, GOLD_D);
  add(6, 10, 4, 1, GOLD);
  add(7, 10, 2, 1, GOLD_L);
  // Sunburst backdrop rays
  add(-2, 6, 1, 1, GOLD_L);
  add(-2, 9, 1, 1, GOLD);
  add(-2, 12, 1, 1, GOLD_L);
  add(17, 6, 1, 1, GOLD_L);
  add(17, 9, 1, 1, GOLD);
  add(17, 12, 1, 1, GOLD_L);
  // Holy sword on the right side
  add(14, 12, 1, 1, GOLD);
  add(14, 13, 1, 1, GOLD_L);
  add(14, 14, 1, 1, STEEL);
  add(14, 15, 1, 1, STEEL);
  add(14, 16, 1, 1, STEEL);
  add(15, 12, 1, 1, GOLD_D);
  return px;
}

// ─── DEVIL Tier I: Imp ─────────────────────────────────────────────────────
function buildDevilT1(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#c98a7a", SKIN_S = "#8f5b52";
  const ROBE = "#2b0f14", ROBE_L = "#4a1520";
  const CRIMSON = "#c9302c", CRIMSON_D = "#7a1a1a", CRIMSON_L = "#ff6b5c";
  const BLACK = "#0d0608";
  const HORN = "#3a1a1a", HORN_L = "#7a2a2a";
  const EYE_GLOW = "#ff2a2a";
  const GOLD = "#c9a24a";

  add(-1, 5, 1, 2, BLACK);
  add(0, 4, 1, 4, BLACK);
  add(1, 5, 1, 6, CRIMSON_D);
  add(2, 6, 1, 6, CRIMSON_D);
  add(3, 7, 1, 5, CRIMSON_D);
  add(0, 4, 1, 1, HORN);
  add(1, 5, 1, 1, BLACK);
  add(1, 8, 1, 1, BLACK);
  add(1, 10, 1, 1, BLACK);
  add(2, 12, 1, 1, BLACK);
  add(2, 7, 1, 1, CRIMSON);
  add(3, 9, 1, 1, CRIMSON);

  add(16, 5, 1, 2, BLACK);
  add(15, 4, 1, 4, BLACK);
  add(14, 5, 1, 6, CRIMSON_D);
  add(13, 6, 1, 6, CRIMSON_D);
  add(12, 7, 1, 5, CRIMSON_D);
  add(15, 4, 1, 1, HORN);
  add(14, 5, 1, 1, BLACK);
  add(14, 8, 1, 1, BLACK);
  add(14, 10, 1, 1, BLACK);
  add(13, 12, 1, 1, BLACK);
  add(13, 7, 1, 1, CRIMSON);
  add(12, 9, 1, 1, CRIMSON);

  add(4, 8, 8, 7, ROBE);
  add(4, 8, 1, 7, BLACK);
  add(11, 8, 1, 7, BLACK);
  add(3, 15, 1, 1, ROBE);
  add(4, 16, 1, 1, ROBE);
  add(6, 16, 1, 1, ROBE);
  add(8, 16, 1, 1, ROBE);
  add(10, 16, 1, 1, ROBE);
  add(12, 15, 1, 1, ROBE);
  add(6, 8, 4, 1, CRIMSON_D);
  add(7, 9, 2, 1, CRIMSON);
  add(7, 10, 2, 1, GOLD);
  add(7, 11, 2, 1, CRIMSON_D);
  add(3, 9, 1, 4, ROBE);
  add(12, 9, 1, 4, ROBE);
  add(3, 13, 1, 1, SKIN);
  add(12, 13, 1, 1, SKIN);

  add(3, 3, 1, 5, ROBE);
  add(12, 3, 1, 5, ROBE);
  add(4, 2, 8, 1, ROBE);
  add(4, 3, 1, 1, BLACK);
  add(11, 3, 1, 1, BLACK);

  add(7, 7, 2, 1, SKIN);
  add(5, 3, 6, 5, SKIN);
  add(4, 4, 1, 3, SKIN);
  add(11, 4, 1, 3, SKIN);
  add(5, 3, 6, 1, SKIN_S);
  add(5, 7, 1, 1, SKIN_S);
  add(10, 7, 1, 1, SKIN_S);

  add(4, 1, 1, 2, HORN);
  add(3, 0, 1, 1, HORN);
  add(4, 0, 1, 1, HORN_L);
  add(5, 1, 1, 1, HORN_L);
  add(11, 1, 1, 2, HORN);
  add(12, 0, 1, 1, HORN);
  add(11, 0, 1, 1, HORN_L);
  add(10, 1, 1, 1, HORN_L);

  add(4, 5, 1, 1, SKIN);
  add(11, 5, 1, 1, SKIN);

  add(6, 5, 1, 1, EYE_GLOW);
  add(9, 5, 1, 1, EYE_GLOW);
  add(6, 4, 1, 1, CRIMSON_L);
  add(9, 4, 1, 1, CRIMSON_L);

  if (mood === "sad") add(7, 7, 2, 1, BLACK);
  else {
    add(6, 6, 3, 1, BLACK);
    add(9, 7, 1, 1, BLACK);
  }
  add(6, 7, 1, 1, "#f5e5c0");
  add(8, 7, 1, 1, "#f5e5c0");

  add(13, 8, 1, 1, GOLD);
  add(15, 8, 1, 1, GOLD);
  add(14, 8, 1, 1, CRIMSON_L);
  add(14, 7, 1, 1, GOLD);
  add(14, 9, 1, 5, "#5a3a1a");
  add(14, 14, 1, 1, GOLD);

  add(12, 15, 1, 1, ROBE_L);
  add(13, 15, 1, 1, ROBE_L);
  add(13, 16, 1, 1, CRIMSON);
  add(14, 16, 1, 1, CRIMSON_L);

  return px;
}

// ─── DEVIL Tier II: Fiend — molten cracks, bigger horns, tail flame ─────────
function buildDevilT2(mood: CompanionMood): Px[] {
  const px = buildDevilT1(mood);
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const EMBER = "#ffb147", CRIMSON_L = "#ff6b5c", CRIMSON = "#c9302c", HORN_L = "#7a2a2a";
  // Larger, curled horns
  add(3, -1, 1, 1, HORN_L);
  add(12, -1, 1, 1, HORN_L);
  add(2, 0, 1, 1, "#3a1a1a");
  add(13, 0, 1, 1, "#3a1a1a");
  // Molten cracks on chest
  add(5, 10, 1, 1, EMBER);
  add(10, 10, 1, 1, EMBER);
  add(6, 12, 1, 1, CRIMSON_L);
  add(9, 12, 1, 1, CRIMSON_L);
  // Ember particles floating up
  add(-1, 3, 1, 1, EMBER);
  add(17, 3, 1, 1, EMBER);
  add(-2, 8, 1, 1, CRIMSON_L);
  add(17, 8, 1, 1, CRIMSON_L);
  // Tail flame
  add(15, 16, 1, 1, EMBER);
  add(15, 15, 1, 1, CRIMSON);
  return px;
}

// ─── DEVIL Tier III: Archdemon — massive wings, crown of fire, hellfire ─────
function buildDevilT3(mood: CompanionMood): Px[] {
  const px = buildDevilT2(mood);
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const CRIMSON = "#c9302c", CRIMSON_L = "#ff6b5c", EMBER = "#ffb147", GOLD = "#c9a24a";
  const BLACK = "#0d0608";
  // Massive extended wing spans
  add(-2, 4, 1, 8, BLACK);
  add(-2, 6, 1, 1, CRIMSON);
  add(-2, 9, 1, 1, CRIMSON);
  add(17, 4, 1, 8, BLACK);
  add(17, 6, 1, 1, CRIMSON);
  add(17, 9, 1, 1, CRIMSON);
  // Crown of fire
  add(5, -1, 1, 1, CRIMSON);
  add(6, -2, 1, 1, EMBER);
  add(8, -2, 1, 1, EMBER);
  add(10, -2, 1, 1, EMBER);
  add(11, -1, 1, 1, CRIMSON);
  add(7, -1, 1, 1, GOLD);
  add(9, -1, 1, 1, GOLD);
  // Hellfire aura on ground
  add(3, 17, 2, 1, CRIMSON);
  add(6, 17, 4, 1, EMBER);
  add(11, 17, 2, 1, CRIMSON);
  // Red lightning around feet
  add(2, 16, 1, 1, CRIMSON_L);
  add(13, 16, 1, 1, CRIMSON_L);
  return px;
}

// ─── PHOENIX ────────────────────────────────────────────────────────────────
function buildPhoenix(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#ffd6a5", SKIN_S = "#c98a5a";
  const FIRE_D = "#7a1500", FIRE = "#e63900", FIRE_L = "#ff7a1a";
  const EMBER = "#ffd23a", WHITE_HOT = "#fff2a8";
  const ROBE = "#2a0a0a", GOLD = "#ffb800";

  add(-1, 6, 1, 2, FIRE_D);
  add(0, 5, 1, 5, FIRE_D);
  add(1, 4, 1, 8, FIRE);
  add(2, 5, 1, 8, FIRE_L);
  add(3, 7, 1, 5, EMBER);
  add(0, 4, 1, 1, EMBER);
  add(1, 3, 1, 1, WHITE_HOT);
  add(1, 12, 1, 1, EMBER);
  add(2, 4, 1, 1, WHITE_HOT);
  add(3, 6, 1, 1, WHITE_HOT);
  add(3, 12, 1, 1, EMBER);
  add(16, 6, 1, 2, FIRE_D);
  add(15, 5, 1, 5, FIRE_D);
  add(14, 4, 1, 8, FIRE);
  add(13, 5, 1, 8, FIRE_L);
  add(12, 7, 1, 5, EMBER);
  add(15, 4, 1, 1, EMBER);
  add(14, 3, 1, 1, WHITE_HOT);
  add(14, 12, 1, 1, EMBER);
  add(13, 4, 1, 1, WHITE_HOT);
  add(12, 6, 1, 1, WHITE_HOT);
  add(12, 12, 1, 1, EMBER);

  add(4, 8, 8, 7, ROBE);
  add(4, 8, 1, 7, "#180505");
  add(11, 8, 1, 7, "#180505");
  add(4, 8, 8, 1, FIRE_D);
  add(5, 9, 6, 1, FIRE);
  add(6, 10, 4, 1, FIRE_L);
  add(4, 12, 8, 1, FIRE_D);
  add(4, 13, 8, 1, FIRE);
  add(5, 14, 6, 1, EMBER);
  add(3, 9, 1, 4, ROBE);
  add(12, 9, 1, 4, ROBE);
  add(3, 12, 1, 1, FIRE);
  add(12, 12, 1, 1, FIRE);
  add(3, 13, 1, 1, SKIN);
  add(12, 13, 1, 1, SKIN);
  add(4, 15, 8, 1, FIRE_D);
  add(4, 16, 1, 1, FIRE);
  add(6, 16, 1, 1, FIRE);
  add(8, 16, 1, 1, FIRE);
  add(10, 16, 1, 1, FIRE);
  add(11, 16, 1, 1, FIRE);

  add(7, 7, 2, 1, SKIN);
  add(5, 3, 6, 5, SKIN);
  add(4, 4, 1, 3, SKIN);
  add(11, 4, 1, 3, SKIN);
  add(5, 7, 1, 1, SKIN_S);
  add(10, 7, 1, 1, SKIN_S);

  add(4, 2, 8, 1, FIRE_D);
  add(4, 1, 1, 1, FIRE);
  add(6, 0, 1, 2, FIRE);
  add(8, 0, 1, 2, EMBER);
  add(10, 0, 1, 2, FIRE);
  add(11, 1, 1, 1, FIRE);
  add(5, 1, 1, 1, EMBER);
  add(9, 1, 1, 1, WHITE_HOT);
  add(7, 1, 1, 1, WHITE_HOT);
  add(7, 2, 2, 1, GOLD);

  add(6, 5, 1, 1, EMBER);
  add(9, 5, 1, 1, EMBER);
  add(6, 4, 1, 1, WHITE_HOT);
  add(9, 4, 1, 1, WHITE_HOT);
  if (mood === "sad") add(7, 7, 2, 1, "#7a1500");
  else add(6, 6, 4, 1, "#7a1500");

  return px;
}

// ─── VOID ───────────────────────────────────────────────────────────────────
function buildVoid(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#a68eb8", SKIN_S = "#5f3f7a";
  const ROBE = "#0a0518", ROBE_L = "#1a0f3d";
  const PURPLE = "#5b2ea8", PURPLE_L = "#8b5cf6", MAGENTA = "#c026d3", CYAN = "#22d3ee";
  const STAR = "#ffffff", GOLD = "#e0a63a";

  add(0, 4, 1, 9, ROBE_L);
  add(1, 3, 1, 11, PURPLE);
  add(2, 5, 1, 10, PURPLE_L);
  add(3, 7, 1, 6, MAGENTA);
  add(0, 5, 1, 1, STAR);
  add(1, 8, 1, 1, STAR);
  add(2, 6, 1, 1, CYAN);
  add(2, 11, 1, 1, STAR);
  add(0, 10, 1, 1, CYAN);
  add(16, 4, 1, 9, ROBE_L);
  add(15, 3, 1, 11, PURPLE);
  add(14, 5, 1, 10, PURPLE_L);
  add(13, 7, 1, 6, MAGENTA);
  add(16, 5, 1, 1, STAR);
  add(15, 8, 1, 1, STAR);
  add(14, 6, 1, 1, CYAN);
  add(14, 11, 1, 1, STAR);
  add(16, 10, 1, 1, CYAN);

  add(4, 8, 8, 7, ROBE);
  add(4, 8, 1, 7, "#000000");
  add(11, 8, 1, 7, "#000000");
  add(6, 8, 4, 1, PURPLE);
  add(7, 9, 2, 1, PURPLE_L);
  add(7, 10, 2, 2, CYAN);
  add(7, 10, 1, 1, PURPLE_L);
  add(8, 11, 1, 1, MAGENTA);
  add(4, 13, 8, 1, PURPLE);
  add(5, 14, 6, 1, MAGENTA);
  add(3, 9, 1, 4, ROBE);
  add(12, 9, 1, 4, ROBE);
  add(3, 13, 1, 1, SKIN);
  add(12, 13, 1, 1, SKIN);
  add(5, 11, 1, 1, STAR);
  add(10, 12, 1, 1, STAR);
  add(6, 14, 1, 1, CYAN);
  add(9, 11, 1, 1, STAR);
  add(4, 15, 8, 1, ROBE_L);
  add(4, 16, 8, 1, PURPLE);

  add(7, 7, 2, 1, SKIN);
  add(5, 3, 6, 5, SKIN);
  add(4, 4, 1, 3, SKIN);
  add(11, 4, 1, 3, SKIN);
  add(5, 7, 1, 1, SKIN_S);
  add(10, 7, 1, 1, SKIN_S);
  add(5, 3, 6, 1, SKIN_S);

  add(3, 3, 1, 5, ROBE);
  add(12, 3, 1, 5, ROBE);
  add(4, 2, 8, 1, ROBE);
  add(3, 3, 1, 1, PURPLE);
  add(12, 3, 1, 1, PURPLE);

  add(4, 0, 1, 1, STAR);
  add(6, 0, 1, 1, CYAN);
  add(8, 0, 1, 1, MAGENTA);
  add(10, 0, 1, 1, CYAN);
  add(11, 0, 1, 1, STAR);
  add(7, 1, 1, 1, GOLD);
  add(9, 1, 1, 1, GOLD);

  add(6, 5, 1, 1, CYAN);
  add(9, 5, 1, 1, CYAN);
  add(6, 4, 1, 1, PURPLE_L);
  add(9, 4, 1, 1, PURPLE_L);
  if (mood === "sad") add(7, 7, 2, 1, PURPLE);
  else add(7, 6, 2, 1, PURPLE_L);

  return px;
}

// ─── TITAN ──────────────────────────────────────────────────────────────────
function buildTitan(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#f0c9a4", SKIN_S = "#b78862";
  const GOLD = "#f2c94c", GOLD_D = "#b8860b", GOLD_L = "#fff2a8";
  const BRONZE = "#c17817", ARMOR = "#d4a017", ARMOR_D = "#7a5010";
  const CAPE = "#7a1a1a", CAPE_D = "#3d0a0a", HAIR = "#c9924a";

  add(2, 3, 1, 1, GOLD);
  add(1, 4, 1, 1, GOLD_D);
  add(1, 6, 1, 1, GOLD);
  add(1, 8, 1, 1, GOLD_D);
  add(2, 10, 1, 1, GOLD);
  add(13, 3, 1, 1, GOLD);
  add(14, 4, 1, 1, GOLD_D);
  add(14, 6, 1, 1, GOLD);
  add(14, 8, 1, 1, GOLD_D);
  add(13, 10, 1, 1, GOLD);
  add(2, 1, 1, 1, GOLD_L);
  add(13, 1, 1, 1, GOLD_L);
  add(0, 5, 1, 1, GOLD_L);
  add(15, 5, 1, 1, GOLD_L);

  add(1, 9, 1, 6, CAPE_D);
  add(2, 8, 1, 8, CAPE);
  add(3, 8, 1, 8, CAPE);
  add(12, 8, 1, 8, CAPE);
  add(13, 8, 1, 8, CAPE);
  add(14, 9, 1, 6, CAPE_D);

  add(4, 8, 8, 7, ARMOR);
  add(4, 8, 1, 7, ARMOR_D);
  add(11, 8, 1, 7, ARMOR_D);
  add(5, 9, 2, 2, GOLD);
  add(9, 9, 2, 2, GOLD);
  add(6, 9, 1, 1, GOLD_L);
  add(10, 9, 1, 1, GOLD_L);
  add(7, 10, 2, 1, ARMOR_D);
  add(7, 11, 2, 2, BRONZE);
  add(7, 11, 1, 1, GOLD_L);
  add(8, 12, 1, 1, GOLD);
  add(4, 13, 8, 1, ARMOR_D);
  add(4, 14, 8, 1, BRONZE);
  add(7, 14, 2, 1, GOLD_L);
  add(3, 8, 1, 2, GOLD);
  add(12, 8, 1, 2, GOLD);
  add(3, 8, 1, 1, GOLD_L);
  add(12, 8, 1, 1, GOLD_L);
  add(3, 10, 1, 3, ARMOR);
  add(12, 10, 1, 3, ARMOR);
  add(3, 13, 1, 1, SKIN);
  add(12, 13, 1, 1, SKIN);
  add(5, 15, 2, 1, GOLD);
  add(9, 15, 2, 1, GOLD);

  add(7, 7, 2, 1, SKIN);
  add(5, 3, 6, 5, SKIN);
  add(4, 4, 1, 3, SKIN);
  add(11, 4, 1, 3, SKIN);
  add(5, 7, 1, 1, SKIN_S);
  add(10, 7, 1, 1, SKIN_S);

  add(4, 2, 8, 2, HAIR);
  add(5, 2, 6, 1, GOLD_L);
  add(4, 3, 1, 2, HAIR);
  add(11, 3, 1, 2, HAIR);

  add(3, 2, 1, 1, "#1a5c2a");
  add(4, 1, 1, 1, "#1a5c2a");
  add(5, 0, 1, 1, "#2a8040");
  add(7, 0, 1, 1, "#2a8040");
  add(9, 0, 1, 1, "#2a8040");
  add(10, 1, 1, 1, "#1a5c2a");
  add(11, 2, 1, 1, "#1a5c2a");
  add(6, 1, 1, 1, "#2a8040");
  add(8, 1, 1, 1, "#2a8040");
  add(7, 2, 2, 1, GOLD_L);

  add(6, 5, 1, 1, GOLD_D);
  add(9, 5, 1, 1, GOLD_D);
  add(6, 4, 1, 1, GOLD);
  add(9, 4, 1, 1, GOLD);
  if (mood === "sad") add(7, 7, 2, 1, ARMOR_D);
  else add(6, 6, 4, 1, "#5a2a1a");

  return px;
}

// ─── PROFESSOR ──────────────────────────────────────────────────────────────
function buildProfessor(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#f0c9a4", SKIN_S = "#c8956f";
  const HAIR = "#d7dbe0", HAIR_D = "#9aa3ad";
  const BLAZER = "#182338", BLAZER_L = "#263a5c", CHECK = "#6f7b91", SHIRT = "#dbeafe";
  const MIC_BLUE = "#174eb6", MIC_ORANGE = "#f59e0b", MIC_RED = "#dc2626", MIC_CYAN = "#06b6d4";
  const GOLD = "#f2c94c", BACKDROP = "#1f68b3";

  add(-1, 3, 1, 9, BACKDROP);
  add(16, 3, 1, 9, BACKDROP);
  add(0, 2, 1, 1, "#8cc7ff");
  add(15, 2, 1, 1, "#8cc7ff");
  add(1, 5, 1, 1, "#5aa7f0");
  add(14, 6, 1, 1, "#5aa7f0");

  add(3, 8, 10, 7, BLAZER);
  add(3, 8, 1, 7, "#0e1728");
  add(12, 8, 1, 7, "#0e1728");
  add(4, 9, 8, 1, BLAZER_L);
  add(5, 8, 1, 7, CHECK);
  add(10, 8, 1, 7, CHECK);
  add(4, 11, 8, 1, CHECK);
  add(4, 14, 8, 1, CHECK);
  add(6, 8, 4, 6, SHIRT);
  add(5, 8, 1, 2, "#ffffff");
  add(10, 8, 1, 2, "#ffffff");
  add(7, 9, 2, 1, "#b9d5f8");
  add(7, 10, 2, 3, "#c9ddf5");
  add(4, 8, 2, 4, BLAZER_L);
  add(10, 8, 2, 4, BLAZER_L);
  add(5, 12, 1, 1, GOLD);
  add(2, 10, 1, 4, BLAZER);
  add(13, 10, 1, 4, BLAZER);
  add(3, 13, 1, 1, SKIN);
  add(12, 13, 1, 1, SKIN);

  add(4, 15, 2, 2, MIC_BLUE);
  add(7, 14, 2, 3, MIC_ORANGE);
  add(10, 15, 2, 2, MIC_RED);
  add(13, 14, 2, 3, MIC_CYAN);
  add(5, 14, 1, 1, "#ffffff");
  add(8, 15, 1, 1, "#ffffff");
  add(11, 14, 1, 1, "#ffffff");
  add(14, 15, 1, 1, "#ffffff");
  add(5, 13, 1, 2, "#3f3f46");
  add(8, 13, 1, 2, "#3f3f46");
  add(11, 13, 1, 2, "#3f3f46");
  add(14, 13, 1, 2, "#3f3f46");

  add(7, 7, 2, 1, SKIN);
  add(5, 3, 6, 5, SKIN);
  add(4, 4, 1, 3, SKIN);
  add(11, 4, 1, 3, SKIN);
  add(5, 7, 1, 1, SKIN_S);
  add(10, 7, 1, 1, SKIN_S);
  add(8, 5, 1, 1, SKIN_S);

  add(4, 2, 8, 1, HAIR);
  add(4, 3, 2, 1, HAIR_D);
  add(10, 3, 2, 1, HAIR_D);
  add(4, 4, 1, 1, HAIR);
  add(11, 4, 1, 1, HAIR);
  add(5, 2, 2, 1, "#eef2f6");
  add(9, 2, 2, 1, "#eef2f6");
  add(7, 1, 2, 1, "#f8fafc");

  add(6, 5, 1, 1, "#172033");
  add(9, 5, 1, 1, "#172033");
  add(6, 4, 1, 1, HAIR_D);
  add(9, 4, 1, 1, HAIR_D);
  if (mood === "happy") add(6, 6, 4, 1, "#8a4b45");
  else if (mood === "sad") add(7, 7, 2, 1, "#8a4b45");
  else add(7, 6, 2, 1, "#8a4b45");

  add(5, 0, 1, 1, GOLD);
  add(7, 0, 1, 1, "#fff2a8");
  add(9, 0, 1, 1, GOLD);
  add(10, 1, 1, 1, GOLD);
  add(5, 1, 5, 1, GOLD);

  return px;
}

// ─── SHADOW REAPER ──────────────────────────────────────────────────────────
function buildReaper(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const OBSIDIAN = "#050308", ROBE = "#0d0812", ROBE_L = "#1a0f2a";
  const VIOLET = "#8b3af8", VIOLET_L = "#c78cff", VIOLET_D = "#4a1a8a";
  const SMOKE = "#2a1a3f", SMOKE_L = "#4a2a5f";
  const BONE = "#e8e4d8", BONE_D = "#8f8a7a";
  const EYE = "#ff2f4a";
  const SILVER = "#c0c8d4";

  // Smoke drift behind
  add(-1, 2, 1, 3, SMOKE);
  add(16, 2, 1, 3, SMOKE);
  add(0, 5, 1, 3, SMOKE_L);
  add(15, 5, 1, 3, SMOKE_L);
  add(-1, 10, 1, 4, SMOKE);
  add(16, 10, 1, 4, SMOKE);
  add(-2, 14, 1, 3, SMOKE_L);
  add(17, 14, 1, 3, SMOKE_L);

  // Long swaying hooded robe
  add(3, 2, 10, 1, OBSIDIAN);
  add(2, 3, 12, 5, ROBE);
  add(2, 8, 12, 8, ROBE);
  add(1, 4, 1, 12, ROBE_L);
  add(14, 4, 1, 12, ROBE_L);
  add(2, 16, 12, 1, OBSIDIAN);
  // Tattered hem
  add(2, 17, 1, 1, ROBE);
  add(4, 17, 1, 1, ROBE);
  add(6, 17, 1, 1, ROBE);
  add(8, 17, 1, 1, ROBE);
  add(10, 17, 1, 1, ROBE);
  add(12, 17, 1, 1, ROBE);
  // Violet inner glow
  add(6, 9, 4, 1, VIOLET_D);
  add(7, 10, 2, 3, VIOLET);
  add(7, 10, 2, 1, VIOLET_L);

  // Void head shadow — no visible face, just glowing eyes
  add(5, 4, 6, 4, "#000000");
  add(4, 5, 1, 2, OBSIDIAN);
  add(11, 5, 1, 2, OBSIDIAN);
  // Glowing red eyes
  add(6, 6, 1, 1, EYE);
  add(9, 6, 1, 1, EYE);
  add(6, 5, 1, 1, "#5a0810");
  add(9, 5, 1, 1, "#5a0810");

  // Scythe — bone shaft on the right, curved violet blade
  add(15, 5, 1, 1, VIOLET_L);
  add(14, 5, 1, 1, VIOLET);
  add(13, 5, 1, 1, VIOLET);
  add(12, 5, 1, 1, VIOLET_D);
  add(13, 4, 1, 1, VIOLET_L);
  add(14, 4, 1, 1, VIOLET_L);
  add(15, 6, 1, 1, BONE);
  add(15, 7, 1, 1, BONE);
  add(15, 8, 1, 1, BONE);
  add(15, 9, 1, 1, BONE_D);
  add(15, 10, 1, 1, BONE);
  add(15, 11, 1, 1, BONE_D);
  add(15, 12, 1, 1, BONE);
  add(15, 13, 1, 1, BONE_D);
  add(15, 14, 1, 1, SILVER);

  // Skeletal hands
  add(3, 12, 1, 1, BONE);
  add(3, 13, 1, 1, BONE);

  // Cracked ground shadow
  add(2, 18, 1, 1, VIOLET_D);
  add(4, 18, 1, 1, VIOLET);
  add(6, 18, 1, 1, VIOLET_L);
  add(9, 18, 1, 1, VIOLET);
  add(11, 18, 1, 1, VIOLET_D);
  add(13, 18, 1, 1, VIOLET);

  if (mood === "happy") add(5, 8, 6, 1, VIOLET_L);
  return px;
}

// ─── CELESTIAL ORACLE ───────────────────────────────────────────────────────
function buildOracle(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#f8e0c2", SKIN_S = "#c9a682";
  const HAIR = "#f7f1de", HAIR_L = "#fffbee";
  const ROBE = "#f8f4e8", ROBE_S = "#e0d7b8";
  const GOLD = "#e8b840", GOLD_L = "#ffe088", GOLD_D = "#8f6a1a";
  const CYAN = "#7fd8ff", CYAN_L = "#c2edff";
  const STAR = "#ffffff", TEAL = "#3fb9c8";
  const LILAC = "#c7a8ff", AURORA = "#8fd4b3";

  // Aurora aura background
  add(-1, 2, 1, 6, AURORA);
  add(-1, 8, 1, 5, LILAC);
  add(16, 2, 1, 6, LILAC);
  add(16, 8, 1, 5, AURORA);
  add(-2, 6, 1, 1, CYAN_L);
  add(17, 6, 1, 1, CYAN_L);

  // Twin star orbs orbiting shoulders
  add(1, 4, 1, 1, STAR);
  add(14, 4, 1, 1, STAR);
  add(1, 3, 1, 1, CYAN);
  add(14, 3, 1, 1, CYAN);

  // Flowing priestess robe
  add(3, 8, 10, 8, ROBE);
  add(3, 8, 1, 8, ROBE_S);
  add(12, 8, 1, 8, ROBE_S);
  add(2, 10, 1, 5, ROBE);
  add(13, 10, 1, 5, ROBE);
  add(4, 8, 8, 1, GOLD);
  add(5, 9, 6, 1, GOLD_L);
  // Runed sash
  add(3, 11, 10, 1, GOLD_D);
  add(5, 11, 1, 1, STAR);
  add(7, 11, 1, 1, STAR);
  add(9, 11, 1, 1, STAR);
  add(11, 11, 1, 1, STAR);
  // Hem details
  add(3, 15, 10, 1, GOLD);
  add(4, 16, 8, 1, GOLD_L);
  add(3, 16, 1, 1, GOLD);
  add(12, 16, 1, 1, GOLD);

  // Floating spellbook in front
  add(6, 13, 4, 2, "#3b2a5a");
  add(6, 13, 4, 1, LILAC);
  add(7, 14, 2, 1, GOLD_L);
  add(8, 14, 1, 1, STAR);

  // Face
  add(7, 7, 2, 1, SKIN);
  add(5, 3, 6, 5, SKIN);
  add(4, 4, 1, 3, SKIN);
  add(11, 4, 1, 3, SKIN);
  add(5, 7, 1, 1, SKIN_S);
  add(10, 7, 1, 1, SKIN_S);

  // Long silver-white hair
  add(4, 2, 8, 2, HAIR);
  add(5, 2, 6, 1, HAIR_L);
  add(3, 4, 1, 4, HAIR);
  add(12, 4, 1, 4, HAIR);
  add(2, 6, 1, 3, HAIR);
  add(13, 6, 1, 3, HAIR);

  // Halo of rotating runes (baseline positions; CSS rotates the ring)
  add(3, 0, 1, 1, GOLD);
  add(6, -1, 1, 1, CYAN);
  add(8, -2, 1, 1, GOLD_L);
  add(10, -1, 1, 1, CYAN);
  add(13, 0, 1, 1, GOLD);
  add(4, 1, 1, 1, STAR);
  add(12, 1, 1, 1, STAR);

  // Serene glowing cyan eyes
  add(6, 5, 1, 1, TEAL);
  add(9, 5, 1, 1, TEAL);
  add(6, 4, 1, 1, CYAN_L);
  add(9, 4, 1, 1, CYAN_L);

  if (mood === "sad") add(7, 7, 2, 1, "#a8846b");
  else add(7, 6, 2, 1, "#c98a7a");

  return px;
}

// ─── CYBER SAMURAI ──────────────────────────────────────────────────────────
function buildSamurai(mood: CompanionMood): Px[] {
  const px: Px[] = [];
  const add = (x: number, y: number, w: number, h: number, c: string) => px.push({ x, y, w, h, c });
  const SKIN = "#e6b790", SKIN_S = "#8a6552";
  const PLATE = "#1a2432", PLATE_L = "#2d3f5a", PLATE_D = "#0a1018";
  const NEON = "#22e8f0", NEON_D = "#0aa8b8", NEON_L = "#a8fdff";
  const HOLO = "#ff5ac9", HOLO_L = "#ffb3e8";
  const CIRCUIT = "#3affd4";
  const BLADE = "#b6f7ff", BLADE_D = "#4fb7c0", BLADE_L = "#e8fbff";
  const HAIR = "#111318";

  // Neon backdrop stripes
  add(-1, 4, 1, 2, NEON_D);
  add(-1, 10, 1, 2, HOLO);
  add(16, 4, 1, 2, NEON_D);
  add(16, 10, 1, 2, HOLO);
  add(-2, 7, 1, 1, NEON);
  add(17, 7, 1, 1, NEON);

  // Armored torso
  add(3, 8, 10, 7, PLATE);
  add(3, 8, 1, 7, PLATE_D);
  add(12, 8, 1, 7, PLATE_D);
  add(4, 9, 8, 1, PLATE_L);
  add(5, 10, 6, 1, PLATE_L);
  // Chest crest — animated circuits (drawn as bright pixels)
  add(6, 10, 4, 1, CIRCUIT);
  add(6, 11, 1, 1, NEON);
  add(9, 11, 1, 1, NEON);
  add(7, 12, 2, 2, NEON_D);
  add(7, 12, 2, 1, NEON);
  add(8, 13, 1, 1, NEON_L);
  // Belt / obi
  add(3, 14, 10, 1, HOLO);
  add(4, 15, 8, 1, PLATE_D);
  // Pauldrons
  add(2, 8, 2, 2, PLATE_L);
  add(12, 8, 2, 2, PLATE_L);
  add(2, 8, 1, 1, NEON);
  add(13, 8, 1, 1, NEON);
  // Arms
  add(2, 10, 1, 4, PLATE);
  add(13, 10, 1, 4, PLATE);
  add(2, 13, 1, 1, SKIN);
  add(13, 13, 1, 1, SKIN);
  // Hovering ember thrust trail
  add(4, 17, 1, 1, HOLO);
  add(6, 17, 1, 1, NEON);
  add(9, 17, 1, 1, NEON);
  add(11, 17, 1, 1, HOLO);
  add(7, 16, 2, 1, NEON_L);

  // Head — helm with HUD reticle
  add(4, 3, 8, 4, PLATE);
  add(4, 3, 8, 1, PLATE_L);
  add(5, 4, 6, 3, SKIN);
  add(5, 4, 6, 1, SKIN_S);
  add(4, 4, 1, 3, PLATE_D);
  add(11, 4, 1, 3, PLATE_D);
  add(4, 7, 8, 1, PLATE_D);
  // Menpo mask lower
  add(5, 7, 6, 1, HOLO);
  add(6, 8, 4, 1, PLATE_D);
  // Topknot / spiked hair
  add(6, 1, 4, 1, HAIR);
  add(7, 0, 2, 1, HAIR);
  add(5, 2, 6, 1, HAIR);
  add(6, 0, 1, 1, NEON);
  add(9, 0, 1, 1, NEON);

  // Eyes: one normal cyan, one HUD reticle
  add(6, 5, 1, 1, NEON);
  add(9, 5, 1, 1, NEON_L);
  // HUD reticle overlay around right eye
  add(8, 5, 1, 1, HOLO);
  add(10, 5, 1, 1, HOLO);
  add(9, 4, 1, 1, HOLO_L);
  add(9, 6, 1, 1, HOLO_L);

  // Holographic katana held on right side
  add(14, 12, 1, 1, BLADE_D);
  add(14, 11, 1, 1, BLADE);
  add(14, 10, 1, 1, BLADE);
  add(14, 9, 1, 1, BLADE);
  add(14, 8, 1, 1, BLADE_L);
  add(14, 7, 1, 1, NEON_L);
  add(14, 6, 1, 1, NEON);
  add(14, 5, 1, 1, NEON);
  add(15, 4, 1, 1, NEON_L);
  add(13, 13, 1, 1, HOLO);
  // Scanline flicker overlay
  add(6, 3, 4, 1, NEON_L);

  if (mood === "sad") add(7, 8, 2, 1, PLATE_D);
  return px;
}

const SPECIAL_LABELS: Record<string, string> = {
  angel: "Celestial Healer",
  devil: "Shadow Surgeon",
  phoenix: "Phoenix Reborn",
  void: "Cosmic Wanderer",
  titan: "Golden Titan",
  professor: "Perfect Exam Professor",
  reaper: "Shadow Reaper",
  oracle: "Celestial Oracle",
  samurai: "Cyber Samurai",
};

function auraStops(special: string) {
  switch (special) {
    case "angel":
      return { c1: "#fff2a8", c2: "#f2c94c", c3: "#f2c94c", o1: 0.85, o2: 0.35, blend: "screen" };
    case "devil":
      return { c1: "#ff2a2a", c2: "#7a1a1a", c3: "#2b0f14", o1: 0.65, o2: 0.4, blend: "normal" };
    case "phoenix":
      return { c1: "#ffd23a", c2: "#e63900", c3: "#7a1500", o1: 0.9, o2: 0.5, blend: "screen" };
    case "void":
      return { c1: "#c026d3", c2: "#5b2ea8", c3: "#0a0518", o1: 0.75, o2: 0.45, blend: "screen" };
    case "titan":
      return { c1: "#fff2a8", c2: "#f2c94c", c3: "#b8860b", o1: 0.85, o2: 0.4, blend: "screen" };
    case "professor":
      return { c1: "#8cc7ff", c2: "#1f68b3", c3: "#182338", o1: 0.8, o2: 0.45, blend: "screen" };
    case "reaper":
      return { c1: "#c78cff", c2: "#4a1a8a", c3: "#0d0812", o1: 0.85, o2: 0.55, blend: "screen" };
    case "oracle":
      return { c1: "#c2edff", c2: "#8fd4b3", c3: "#c7a8ff", o1: 0.85, o2: 0.4, blend: "screen" };
    case "samurai":
      return { c1: "#22e8f0", c2: "#ff5ac9", c3: "#0a1018", o1: 0.75, o2: 0.5, blend: "screen" };
    default:
      return { c1: "#fff", c2: "#fff", c3: "#fff", o1: 0, o2: 0, blend: "normal" };
  }
}

export function Companion({
  level,
  mood = "idle",
  size = 96,
  bob = true,
  className,
  character,
}: {
  level: number;
  mood?: CompanionMood;
  size?: number;
  bob?: boolean;
  className?: string;
  character?: CharacterCustomization;
}) {
  const special = character?.special;
  const tier = character?.tier ?? 1;

  const pixels = useMemo(() => {
    if (special === "angel") {
      if (tier >= 3) return buildAngelT3(mood);
      if (tier >= 2) return buildAngelT2(mood);
      return buildAngelT1(mood);
    }
    if (special === "devil") {
      if (tier >= 3) return buildDevilT3(mood);
      if (tier >= 2) return buildDevilT2(mood);
      return buildDevilT1(mood);
    }
    if (special === "phoenix") return buildPhoenix(mood);
    if (special === "void") return buildVoid(mood);
    if (special === "titan") return buildTitan(mood);
    if (special === "professor") return buildProfessor(mood);
    if (special === "reaper") return buildReaper(mood);
    if (special === "oracle") return buildOracle(mood);
    if (special === "samurai") return buildSamurai(mood);
    return buildPixels(level, mood, character);
  }, [level, mood, character, special, tier]);

  const moodClass =
    mood === "happy"
      ? "companion-happy"
      : mood === "sad"
        ? "companion-sad"
        : bob
          ? "companion-bob"
          : "";

  const viewBox = special ? "-2 -2 20 22" : "0 0 16 18";
  const aspectW = special ? 20 : 16;
  const aspectH = special ? 22 : 18;

  const auraId = useMemo(
    () => `aura-${special ?? "n"}-${Math.random().toString(36).slice(2, 7)}`,
    [special],
  );
  const glowId = useMemo(
    () => `glow-${special ?? "n"}-${Math.random().toString(36).slice(2, 7)}`,
    [special],
  );
  const aura = special ? auraStops(special) : null;

  return (
    <svg
      viewBox={viewBox}
      width={size}
      height={size * (aspectH / aspectW)}
      shapeRendering="crispEdges"
      className={cn("pixelated select-none companion-root", moodClass, className)}
      role="img"
      aria-label={
        special
          ? SPECIAL_LABELS[special] + (tier > 1 && (special === "angel" || special === "devil") ? ` — Tier ${tier}` : "")
          : `Study companion, level ${level}`
      }
    >
      {special && aura && (
        <defs>
          <radialGradient id={auraId}>
            <stop offset="0%" stopColor={aura.c1} stopOpacity={aura.o1}>
              <animate attributeName="stop-opacity" values={`${aura.o1};${aura.o1 * 0.55};${aura.o1}`} dur="2.4s" repeatCount="indefinite" />
            </stop>
            <stop offset="60%" stopColor={aura.c2} stopOpacity={aura.o2} />
            <stop offset="100%" stopColor={aura.c3} stopOpacity="0" />
          </radialGradient>
          <radialGradient id={glowId}>
            <stop offset="0%" stopColor="#ffffff" stopOpacity="0.35" />
            <stop offset="100%" stopColor="#ffffff" stopOpacity="0" />
          </radialGradient>
        </defs>
      )}
      {special && aura && (
        <circle
          cx={8}
          cy={9}
          r={12}
          fill={`url(#${auraId})`}
          style={{ mixBlendMode: aura.blend as "screen" | "normal" }}
        >
          <animate attributeName="r" values="11;13;11" dur="3.2s" repeatCount="indefinite" />
        </circle>
      )}
      {pixels.map((px, i) => (
        <rect key={i} x={px.x} y={px.y} width={px.w} height={px.h} fill={px.c} />
      ))}
      {/* Ambient particles per form (positions vary by special) */}
      {special === "angel" && (
        <g className="companion-sparkle-slow">
          <rect x={-1} y={2} width={1} height={1} fill="#ffffff" opacity="0.9" />
          <rect x={17} y={1} width={1} height={1} fill="#ffffff" opacity="0.9" />
          <rect x={2} y={15} width={1} height={1} fill="#fff2a8" opacity="0.9" />
          <rect x={15} y={14} width={1} height={1} fill="#fff2a8" opacity="0.9" />
        </g>
      )}
      {special === "devil" && (
        <g className="companion-ember">
          <rect x={-1} y={16} width={1} height={1} fill="#ff6b5c" opacity="0.85" />
          <rect x={2} y={17} width={1} height={1} fill="#c9302c" opacity="0.85" />
          <rect x={15} y={17} width={1} height={1} fill="#ff6b5c" opacity="0.85" />
          <rect x={17} y={15} width={1} height={1} fill="#c9302c" opacity="0.85" />
        </g>
      )}
      {special === "phoenix" && (
        <g className="companion-ember">
          <rect x={-1} y={2} width={1} height={1} fill="#ffd23a" opacity="0.9" />
          <rect x={17} y={1} width={1} height={1} fill="#ff7a1a" opacity="0.9" />
          <rect x={-1} y={13} width={1} height={1} fill="#ff7a1a" opacity="0.9" />
          <rect x={17} y={14} width={1} height={1} fill="#ffd23a" opacity="0.9" />
          <rect x={4} y={17} width={1} height={1} fill="#e63900" opacity="0.9" />
          <rect x={11} y={17} width={1} height={1} fill="#e63900" opacity="0.9" />
        </g>
      )}
      {special === "void" && (
        <g className="companion-sparkle-slow">
          <rect x={-1} y={1} width={1} height={1} fill="#ffffff" opacity="0.9" />
          <rect x={17} y={2} width={1} height={1} fill="#22d3ee" opacity="0.9" />
          <rect x={-2} y={9} width={1} height={1} fill="#c026d3" opacity="0.9" />
          <rect x={17} y={11} width={1} height={1} fill="#ffffff" opacity="0.9" />
          <rect x={5} y={17} width={1} height={1} fill="#22d3ee" opacity="0.9" />
          <rect x={11} y={17} width={1} height={1} fill="#c026d3" opacity="0.9" />
        </g>
      )}
      {special === "titan" && (
        <g className="companion-sparkle-slow">
          <rect x={-1} y={2} width={1} height={1} fill="#fff2a8" opacity="0.95" />
          <rect x={17} y={2} width={1} height={1} fill="#fff2a8" opacity="0.95" />
          <rect x={-2} y={7} width={1} height={1} fill="#f2c94c" opacity="0.9" />
          <rect x={17} y={7} width={1} height={1} fill="#f2c94c" opacity="0.9" />
        </g>
      )}
      {special === "professor" && (
        <g className="companion-sparkle-slow">
          <rect x={-1} y={1} width={1} height={1} fill="#8cc7ff" opacity="0.9" />
          <rect x={17} y={1} width={1} height={1} fill="#8cc7ff" opacity="0.9" />
          <rect x={-2} y={9} width={1} height={1} fill="#f59e0b" opacity="0.9" />
          <rect x={17} y={10} width={1} height={1} fill="#06b6d4" opacity="0.9" />
          <rect x={3} y={17} width={1} height={1} fill="#dc2626" opacity="0.9" />
          <rect x={12} y={17} width={1} height={1} fill="#174eb6" opacity="0.9" />
        </g>
      )}
      {special === "reaper" && (
        <g className="companion-drift">
          <rect x={-1} y={4} width={1} height={1} fill="#c78cff" opacity="0.85" />
          <rect x={17} y={5} width={1} height={1} fill="#8b3af8" opacity="0.8" />
          <rect x={-2} y={11} width={1} height={1} fill="#c78cff" opacity="0.8" />
          <rect x={17} y={12} width={1} height={1} fill="#8b3af8" opacity="0.85" />
          <rect x={4} y={-1} width={1} height={1} fill="#ff2f4a" opacity="0.7" />
          <rect x={11} y={-1} width={1} height={1} fill="#ff2f4a" opacity="0.7" />
        </g>
      )}
      {special === "oracle" && (
        <g className="companion-orbit">
          <rect x={0} y={2} width={1} height={1} fill="#ffffff" opacity="0.9" />
          <rect x={15} y={3} width={1} height={1} fill="#7fd8ff" opacity="0.9" />
          <rect x={-1} y={9} width={1} height={1} fill="#c7a8ff" opacity="0.85" />
          <rect x={17} y={10} width={1} height={1} fill="#8fd4b3" opacity="0.85" />
        </g>
      )}
      {special === "samurai" && (
        <g className="companion-scan">
          <rect x={-1} y={3} width={1} height={1} fill="#22e8f0" opacity="0.85" />
          <rect x={17} y={3} width={1} height={1} fill="#ff5ac9" opacity="0.85" />
          <rect x={-2} y={11} width={1} height={1} fill="#ff5ac9" opacity="0.9" />
          <rect x={17} y={12} width={1} height={1} fill="#22e8f0" opacity="0.9" />
          <rect x={5} y={17} width={1} height={1} fill="#a8fdff" opacity="0.9" />
          <rect x={11} y={17} width={1} height={1} fill="#a8fdff" opacity="0.9" />
        </g>
      )}
    </svg>
  );
}
